...
use crate::mood::{Mood, RepoMetrics};
use panels::Panel;
use ratatui::{
...
impl ActivePanel {
    pub fn index(&self) -> usize {
        match self {
            ActivePanel::Hotspots => 0,
            ActivePanel::DustyFiles => 1,
            ActivePanel::Todos => 2,
        }
    }
    ...
    pub fn from_index(index: usize) -> Self {
        match index {
            0 => ActivePanel::Hotspots,
            1 => ActivePanel::DustyFiles,
            2 => ActivePanel::Todos,
            _ => ActivePanel::Hotspots,
        }
    }
}
...
impl App {
    ...
    pub fn next_panel(&mut self) {
        self.active_panel = ActivePanel::from_index((self.active_panel.index() + 1) % 3);
        self.scroll_offset = 0;
    }

    pub fn prev_panel(&mut self) {
        self.active_panel = ActivePanel::from_index((self.active_panel.index() + 2) % 3);
        self.scroll_offset = 0;
    }
...
pub fn render(frame: &mut Frame, app: &App) {
    let chunks = Layout::default()
        ...
        .constraints([
            Constraint::Length(4),  // Header with mood
...
fn render_header(frame: &mut Frame, area: Rect, app: &App) {
    let mood_color = app.mood.color();
    
    let header_content = vec![
        Line::from(vec![
            Span::styled(
                format!("  {} ", app.mood.symbol()),
...
fn render_stats(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let stats_line = Line::from(vec![
        ...
        Span::styled(
            format!("{} files", app.metrics.total_files),
            Style::default().fg(Color::White),
        ),
        Span::raw("  │  "),
        Span::styled("📝 ", Style::default().fg(Color::Rgb(52, 211, 153))),
        Span::styled(
            format!("{} changed this week", app.metrics.files_changed_recently),
            Style::default().fg(Color::White),
        ),
        Span::raw("  │  "),
        Span::styled("📌 ", Style::default().fg(Color::Rgb(251, 146, 60))),
        Span::styled(
            format!("{} TODOs", total_todos),
            Style::default().fg(Color::White),
        ),
        Span::raw("  │  "),
        Span::styled("🕸️  ", Style::default().fg(Color::Rgb(148, 163, 184))),
        Span::styled(
            format!("{} dusty", app.metrics.dusty_file_count),
            Style::default().fg(Color::White),
        ),
    ]);
    ...
    let stats = Paragraph::new(stats_line)
        .block(
            Block::default()
                ...
                .border_style(Style::default().fg(Color::Rgb(51, 65, 85)))
                .title(" Stats ")
                .title_style(Style::default().fg(Color::Rgb(148, 163, 184)))
                .style(Style::default().bg(Color::Rgb(15, 23, 42))),
        );
...
fn render_tabs(frame: &mut Frame, area: Rect, app: &App) {
    let titles = vec![
        Line::from(" [1] Hotspots "),
        Line::from(" [2] Dusty Files "),
        Line::from(" [3] TODOs & HACKs "),
    ];
...
fn render_panel(frame: &mut Frame, area: Rect, app: &App) {
    match app.active_panel {
        ActivePanel::Hotspots => {
            let panel = Panel::hotspots(&app.churn_entries, app.scroll_offset, app.mood.color());
            frame.render_widget(panel, area);
        }
        ActivePanel::DustyFiles => {
            let panel = Panel::dusty_files(&app.dusty_files, app.scroll_offset, app.mood.color());
            frame.render_widget(panel, area);
        }
        ActivePanel::Todos => {
            let panel = Panel::todos(&app.todo_entries, app.scroll_offset, app.mood.color());
            frame.render_widget(panel, area);
        }
    }
}
...
fn render_help(frame: &mut Frame, area: Rect, app: &App) {
    let help_line = Line::from(vec![
        Span::styled(" [q] ", Style::default().fg(app.mood.color())),
        Span::raw("quit   "),
        Span::styled("[1-3] ", Style::default().fg(app.mood.color())),
        Span::raw("switch panel   "),
        Span::styled("[↑↓] ", Style::default().fg(app.mood.color())),
        Span::raw("scroll   "),
        Span::styled("[Tab] ", Style::default().fg(app.mood.color())),
        Span::raw("next panel"),
    ]);
