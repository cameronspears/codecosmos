...
fn parse_fix_preview(response: &str, modifier: Option<String>) -> anyhow::Result<FixPreview> {
    ...
    let json_str = match extract_json_object(response) {
        ...
        None => {
            ...
            eprintln!("[DEBUG] LLM response without valid JSON: {}", truncate_str(response, 500));
            return Err(anyhow::anyhow!(
    ...
    let parsed: serde_json::Value = match serde_json::from_str(json_str) {
        ...
        Err(e) => {
            ...
            eprintln!("[DEBUG] Malformed JSON from LLM: {}", truncate_str(json_str, 500));
            ...
            match serde_json::from_str(&fixed) {
                ...
                Err(_) => {
                    ...
                    match try_parse_individual_suggestions(&json_str) {
                        ...
                        _ => {
                            // Log the raw response for debugging (helps troubleshoot model issues)
                            eprintln!("[DEBUG] Failed to parse LLM suggestions: {}", e);
                            eprintln!("[DEBUG] Raw response preview: {}", truncate_str(&json_str, 500));
                            // Return empty with user-friendly message logged
                            eprintln!("Note: The AI returned an unexpected response format. Suggestions may be incomplete.");
                            return Ok(Vec::new());
                        }
                    }
                }
